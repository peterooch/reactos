    ...
    IN DWORD dwCodePage)
{
    ...
    {
        RECT rc = dc->erclWindow;

        IntDPtoLP(dc, (POINT *)&rc, 2);
        Start.x = GetRECTWidth(rc) - (Start.x + 1);
    ...
    else if ((pdcattr->flTextAlign & TA_RIGHT) == TA_RIGHT)
    {
        ...
        if (pdcattr->dwLayout & LAYOUT_RTL)
        {
            ...
            if ((fuOptions & ETO_CLIPPED && lprc) &&
                ((XStart64 + DeltaX64) >> 10) >= lprc->right)
            {
                XStart64 = (lprc->right << 10) - DeltaX64;
            }
            else if (((XStart64 + DeltaX64) >> 10) >= GetRECTWidth(rc))
            {
                XStart64 = (GetRECTWidth(rc) << 10) - DeltaX64; 
            }
        }
    }
    else
